...
describe("PercentEncoder.prototype.encode", () => {
  it("PercentEncoder()/encode", () => {
    ...
    expect(encoder.encode(Uint8Array.of(3,2,1,0,0xFF,0xFE,0xFD,0xFC))).toBe("%03%02%01%00%FF%FE%FD%FC");
    expect(encoder.encode(utf8Bytes1)).toBe("%31%00%20%21%7E%7F%E3%81%82%2B");
    expect(encoder.encode(Uint8Array.of(255))).toBe("%FF");
    expect(encoder.encode(Uint8Array.of(0))).toBe("%00");
    expect(encoder.encode(Uint8Array.of(0,32,65))).toBe("%00%20%41");
    expect(encoder.encode(Uint8Array.of(255))).toBe("%FF");
  ...
  it("PercentEncoder({encodeSet:[]})/encode", () => {
    ...
    expect(encoder.encode(Uint8Array.of())).toBe("");
    expect(encoder.encode(Uint8Array.of(3,2,1,0,0xFF,0xFE,0xFD,0xFC))).toBe("%03%02%01%00%FF%FE%FD%FC");
    expect(encoder.encode(utf8Bytes1)).toBe("1%00 !~%7F%E3%81%82+");
    expect(encoder.encode(Uint8Array.of(255))).toBe("%FF");
    expect(encoder.encode(Uint8Array.of(0))).toBe("%00");
    expect(encoder.encode(Uint8Array.of(0,32,65))).toBe("%00 A");
    expect(encoder.encode(Uint8Array.of(255))).toBe("%FF");
  ...
  it("PercentEncoder({encodeSet:[...]})/encode", () => {
    ...
    expect(encoder.encode(Uint8Array.of())).toBe("");
    expect(encoder.encode(utf8Bytes1)).toBe("1%00%20!~%7F%E3%81%82+");
  ...
  it("PercentEncoder({encodeSet:[...]})/encode", () => {
    ...
    expect(encoder.encode(Uint8Array.of())).toBe("");
    expect(encoder.encode(utf8Bytes1)).toBe("1%00%20!~%7F%E3%81%82%2B");
    expect(encoder.encode(Uint8Array.of(0,32,65))).toBe("%00%20A");
    expect(encoder.encode(Uint8Array.of(255))).toBe("%FF");

    expect(encoder.encode(utf8Bytes1)).toBe(globalThis.encodeURIComponent("1\u{0} !~\u{7F}あ+"));
    ...
    expect(encoder.encode(x1)).toBe(globalThis.encodeURIComponent(x1b));

    expect(encoder.encode(x2bUtf8)).toBe(globalThis.encodeURIComponent(x2b));
  ...
  it("PercentEncoder({encodeSet:[...],spaceAsPlus:true})/encode", () => {
    ...
    expect(encoder.encode(Uint8Array.of())).toBe("");
    expect(encoder.encode(utf8Bytes1)).toBe("1%00+%21%7E%7F%E3%81%82%2B");
    expect(encoder.encode(Uint8Array.of(0,32,65))).toBe("%00+A");
    expect(encoder.encode(Uint8Array.of(255))).toBe("%FF");

    const usp = new URLSearchParams();
    usp.set("p1", "1\u{0} !~\u{7F}あ+");
    expect(encoder.encode(utf8Bytes1)).toBe(usp.toString().substring(3));

    const u = new URL("http://example.com/");
    u.searchParams.set("p1", x2b);
    const e = u.search.replace("?p1=", "");
    expect(encoder.encode(x2bUtf8)).toBe(e);
...
describe("PercentEncoder.get", () => {
  ...
  it("get(Object)", () => {
    ...
    expect(encoder21).toBe(encoder22);
