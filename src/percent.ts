...
const _MIN_OPTIONS: _ResolvedOptions = Object.freeze({
  encodeSet: Object.freeze([] as Array<byte>),
  spaceAsPlus: false,
});

const _URI_FRAGMENT_OPTIONS: _ResolvedOptions = Object.freeze({
  encodeSet: Object.freeze([0x20, 0x22, 0x3C, 0x3E, 0x60] as Array<byte>),
  spaceAsPlus: false,
});

const _URI_QUERY_OPTIONS: _ResolvedOptions = Object.freeze({
  encodeSet: Object.freeze([0x20, 0x22, 0x23, 0x3C, 0x3E] as Array<byte>),
  spaceAsPlus: false,
});

const _URI_SPECIAL_QUERY_OPTIONS: _ResolvedOptions = Object.freeze({
  encodeSet: Object.freeze(
    [0x20, 0x22, 0x23, 0x27, 0x3C, 0x3E] as Array<byte>,
  ),
  spaceAsPlus: false,
});
...
const _URI_PATH_OPTIONS: _ResolvedOptions = Object.freeze({
  encodeSet: Object.freeze(
    [0x20, 0x22, 0x23, 0x3C, 0x3E, 0x3F, 0x60, 0x7B, 0x7D] as Array<byte>,
  ),
  spaceAsPlus: false,
});
  ...
  while (i < encoded.length) {
    ...
    if (c === "%") {
      ...
      if (hexRegExp.test(byteString)) {
        byte = Number.parseInt(byteString, 16) as byte;
        i = i + 3;
      } else {
        byte = c.charCodeAt(0) as byte;
        i = i + 1;
      }
    } else if (c === "+") {
      if (options.spaceAsPlus === true) {
        ...
        byte = 0x2B; // c.charCodeAt(0) as byte;
      }
      i = i + 1;
    } else {
      byte = c.charCodeAt(0) as byte;
...
function _encode(toEncode: Uint8Array, options: _ResolvedOptions): string {
  ...
  for (const byte of toEncode) {
    if (_isByteIncludedInEncodeSet(byte as byte, options.encodeSet)) {
      if (byte === 0x20) {
        if (options.spaceAsPlus === true) {
...
function _resolveOptions(
  ...
  let encodeSet: Readonly<Array<byte>>;
  if (
    Array.isArray(options.encodeSet) &&
    options.encodeSet.every((i) => Byte.isByte(i))
  ) {
    encodeSet = Object.freeze([...(options.encodeSet as byte[])]);
  } else {
